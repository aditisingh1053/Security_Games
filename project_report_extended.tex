...
full-information feedback it costs a factor $\sqrt{\Kmax}$ over the known-$K$
rate of \citet[Theorem 5.1]{balcan2015}. Not observing $a_t$ is affordable
...
\Cref{thm:bits} turns this into an exact accounting. On a universe of $M$
candidate types, and given any oracle emitting $b$ bits over the whole
horizon, the optimal regret is $\min\{T,\log_2 M-b\}\pm 3$: regret is the
number of bits still needed to \emph{name} the attacker, and no finite budget
helps when the universe is infinite. \Cref{prop:sqrtT} shows this naming
account is special to $\Kmax=1$: two types already force $\Omega(\sqrt T)$ on
a two-element universe, under full information and with $K$ known.
...
in the three ways we need: it charges every unsuccessful query, not just the
number of queries until termination; it bounds the whole tail of the success
time; and it lets the searcher be fed a side channel of bounded total width,
which is what \Cref{thm:bits} prices.

\begin{lemma}[comparison trees identify slowly]
\label{lem:count}
Let $M\ge 1$, let $j$ be uniform on $[M]$, and fix a \emph{side-information
budget} $b\ge 0$. Consider any interactive protocol in which, for
$t=1,2,\dots$, a \emph{deterministic} strategy picks a query $q_t$ as a
function of everything seen so far and receives, in order, an answer
$o_t=\mathcal{O}(q_t,j)\in\{1,2,3\}$ and a \emph{side message}
$\mu_t\in\Sigma_t$. The alphabets $\Sigma_0,\Sigma_1,\dots$ are fixed before
play, $\mu_0$ arrives before the first query, each $\mu_t$ is an arbitrary
function of $j$ and the history, and
\begin{equation}
...
\begin{equation}
\Pr[\tau\le u]\;\le\;\frac{2^{b}\,(2^{u}-1)}{M},
\label{eq:tau-tail}
\end{equation}
\begin{equation}
...
\end{equation}
and, in the form easiest to read off,
\begin{equation}
\EE\bigl[\min\{\tau-1,\,T\}\bigr]\;\ge\;
\min\bigl\{T,\ \lfloor\log_2 M\rfloor-b\bigr\}-2 .
\label{eq:count-clean}
\end{equation}
Setting $b=0$ gives the side-channel-free form used in
\Cref{thm:impossible,thm:blocklb}.
\end{lemma}

\begin{proof}
The idea is a counting one: a searcher that has not yet hit has learned only
which of $2^{t-1}$ answer histories it is in, refined by at most $2^{b}$ side
messages, and each such transcript can be the prefix of a hit for at most one
index.

Formally, let $H_t$ be the set of transcripts
$(\mu_0,o_1,\mu_1,\dots,o_{t-1},\mu_{t-1})$ of the first $t-1$ rounds
containing no answer $2$. Each has $o_s\in\{1,3\}$ and $\mu_s\in\Sigma_s$, so
by \Cref{eq:budget}, $|H_t|\le 2^{t-1}\prod_{s<t}|\Sigma_s|\le 2^{t-1}2^{b}$.
If $\tau(j)=t$ then $j$ produces some $h\in H_t$; the strategy being
deterministic, the round-$t$ query is the fixed $q_t(h)$, and
...
$|\{j:\tau(j)=t\}|\le 2^{b}2^{t-1}$. Summing over $t\le u$ and dividing by
$M$ gives \Cref{eq:tau-tail}.
...
\Cref{eq:count-clean} follows from \Cref{eq:count-final} by a short case
analysis on $L=\lfloor\log_2 M\rfloor-b$, deferred to
\Cref{app:count-clean}.
\end{proof}
...
\Cref{lem:hit} gives exactly \Cref{eq:one-hit}, and \Cref{prob:partial}
provides no channel beyond $o_t$, so the budget is $b=0$.

\emph{Counting.} Every round before the first hit $\tau$ is a miss, so by
\Cref{eq:regret-is-misses} and \Cref{eq:count-final} with $b=0$, $M=2^{T}$,
\[
...
\Cref{thm:impossible} is not about how many types the adversary has --- the
hard instance uses one --- but about how many it \emph{could} have had. The
diagnosis is that the defender is missing a \emph{name}, and a name is a
bounded object; so we price the missing supplement in bits.

\begin{definition}[side-information oracle of budget $b$]
\label{def:budget}
A \emph{side-information oracle of budget $b$} for \Cref{prob:partial} is a
family of alphabets $\Sigma_0,\dots,\Sigma_T$ fixed before play with
$\sum_t\log_2|\Sigma_t|\le b$, together with maps handing the defender
$\mu_0\in\Sigma_0$ before round~$1$ and $\mu_t\in\Sigma_t$ at the end of round
$t$. Each $\mu_t$ may be an arbitrary function of the instance $(K,\avec)$
and of the history. Nothing is assumed about what the messages mean; $b$
counts only how many bits of them there are.
\end{definition}

\begin{theorem}[regret is the number of bits needed to name the type]
\label{thm:bits}
Fix $M\ge 2$ and a budget $b\ge 0$, and consider \Cref{prob:partial} in $G_3$
with $\Kmax=1$ plus a side-information oracle of budget $b$. Parts (i) and
(iii) hold for real $b$; part (ii), which builds $2^{b}$ blocks, is stated
for integer $b$.
\begin{enumerate}[leftmargin=1.2em,itemsep=0.05em,topsep=0.2em]
  \item On $\Univ_M$, every algorithm and every such oracle suffer
  $\EE[\Regret(T)]\ge\min\{T,\lfloor\log_2 M\rfloor-b\}-2$ on some instance.
  \item On $\Univ_M$ there are an oracle of budget $b$ and an algorithm
  suffering $\Regret(T)\le\min\{T,\max\{0,\lceil\log_2 M\rceil-b\}\}$ on
  every instance.
  \item On the infinite universe $\Univ$, every algorithm and every oracle of
  \emph{any} finite budget suffer $\EE[\Regret(T)]\ge T-2$ on some instance
  with $|K|=1$.
\end{enumerate}
Hence the optimal regret on $\Univ_M$ is $\min\{T,\log_2 M-b\}\pm 3$, and
$\pm 2$ when $M$ is a power of two. One bit of threat intelligence buys
exactly one round, until the type is named and the rest are free; on an
infinite universe no fixed quantity of intelligence buys anything.
\end{theorem}

\begin{proof}
(i) Run the proof of \Cref{thm:impossible} on $\Univ_M$ verbatim, except that
the protocol handed to \Cref{lem:count} now also carries the messages
$\mu_t$, so its budget is $b$ rather than $0$. This is legitimate because
\Cref{lem:count} lets $q_t$ depend on $\mu_0,\dots,\mu_{t-1}$ --- exactly the
freedom a defender reading the oracle has --- and because \Cref{eq:one-hit}
is a property of $\mathcal{O}$ alone, untouched by the side channel.
Invoking \Cref{eq:count-clean} in place of \Cref{eq:count-final} gives the
claim.

(ii) Write $m=\lceil\log_2 M\rceil$. If $b\ge m$, let the oracle name $j$
outright and the defender play the midpoint of $I_j$ forever: it hits every
round and regret is $0$. Otherwise split $[M]$ into $2^{b}$ consecutive
blocks of at most $\lceil M/2^{b}\rceil\le 2^{m-b}$ indices and let $\mu_0$
name the block containing $j$ --- one message from an alphabet of size
$2^{b}$, so the budget is met. The defender then binary-searches inside that
block: it keeps the consistent sub-interval $[\ell,h]$ and plays
$\pvec=(k/M,0,1-k/M)$ for its midpoint $k$. By \Cref{eq:type-line} the
observation is target~$2$ if $j=k$, target~$1$ if $j>k$ and target~$3$ if
$j<k$, so every miss halves $[\ell,h]$ and every hit leaves it alone. At most
$m-b$ misses occur before $[\ell,h]$ is a singleton, after which every round
hits; and trivially at most $T$ rounds are missed. Apply
\Cref{eq:regret-is-misses}.

(iii) Apply (i) on $\Univ_{M}\subset\Univ$ with $M=2^{T+b}$, for which
$\lfloor\log_2 M\rfloor-b=T$.
\end{proof}

\Cref{def:budget} is deliberately permissive --- the oracle may inspect the
whole instance, speak before play, adapt to the defender, and use any
encoding --- so the lower bound is information-theoretic, not an artefact of
a reporting format. What \Cref{thm:bits} does \emph{not} capture is
\emph{when} the bits arrive; \Cref{thm:blocklb} below shows that even a
generous budget still costs $\Theta(\Kmax B)$ if it may speak only every $B$
rounds. Volume and latency are independent obstructions, and the two lower
bounds isolate them one at a time.

Two further points are worth recording. First, the instance of
\Cref{thm:impossible} is \emph{stationary}, so the bound survives telling the
defender $|K|$, the full frequency vector, or that the sequence is i.i.d.:
none of these is informative when $|K|=1$. What is missing is not which types
attack when --- the quantity \citet[Section 6]{balcan2015} must estimate ---
but the identity of the types. Second, \citet{letchford2009} learn a
near-optimal commitment against a \emph{single} unknown type from few
best-response queries, which only appears to conflict: any such bound must
depend on the geometry of the best-response regions, and $\Univ_M$ drives the
region to width $2^{-T}$. \Cref{thm:bits} says exactly that this dependence
cannot be removed.
...
row 5 is \Cref{thm:impossible}; row 6 is \Cref{thm:bits}; row 7 is
\Cref{prop:sqrtT} with \Cref{thm:main} at $k=2$; row 8 is \Cref{thm:block}.
...
an infinite universe. The $\Omega(\sqrt T)$ half of row 7 holds under partial
...
For $\Kmax=1$ the problem is entirely a naming problem: by \Cref{thm:bits}
the regret stops growing once the type is named, so on a fixed finite
universe it does not grow with $T$ at all. That is special to one type. With
...
nothing; \Cref{fig:impossible}(c) shows the measured curve sitting on that
value.

So \Cref{thm:bits,prop:sqrtT} bracket the finite-universe problem from two
sides, and the transition at $\Kmax=2$ is not about how hard a type is to
identify but about whether identification is the whole problem. On the upper
side, \citet[Theorem 6.1]{balcan2015} with $k=M$ --- legitimate, since a
finite universe \emph{is} a known type set --- gives
...
who its opponents are, and \Cref{thm:bits} says how little may do: what is
missing is a name, and a name is a bounded object. This section supplies the
...
absolute constant $c_2$, whose regret on $[1,L]$ is at most
$c_2 L^{2/3}n|C|\log^{1/3}(n|C|)$ for \emph{every} $L\le T$ simultaneously,
...
\begin{proof}
Adopt the notation of \Cref{def:blocks,lem:blocks}. The horizon splits into
at most $K'$ discovery blocks and at most $K'$ epochs. We charge the former
at face value and run \Cref{lem:anytime} on the latter.

\emph{Epochs.} Fix $j$. By \Cref{lem:blocks}(ii) the defender runs one
uninterrupted instance $\mathcal{A}(C_j)$ --- the anytime wrapper of
\Cref{lem:anytime}, not the horizon-dependent algorithm inside it --- on a
sequence of length $L_j$ drawn from the known set $C_j$, under partial
...
\Cref{lem:blocks}(iii) the epoch is determined by $\avec$ alone. And $L_j$ is
unknown when the instance starts, which is exactly what the anytime form
handles. Hence
\begin{align}
...
\end{align}
Note the left side already compares against the unrestricted maximum over
$\cP$, so the passage through $\cE(C_j;\epsilon)$ is internal to
\citet[Theorem 6.1]{balcan2015} and need not be repeated. Since
$\pvec^{*}\in\cP$ is a feasible competitor in that maximum, the same bound
holds with $\max_{\pvec\in\cP}$ replaced by the value of $\pvec^{*}$ on
$\Ep_j$ --- the substitution of Sub-step 4b of \Cref{sec:proof}.

\emph{Discovery blocks.} Each round costs at most $2$, since
$U_d\in[-1,1]$, and there are at most $K'B$ such rounds, for a total of
$2K'B$. We do not try to learn there, and \Cref{thm:blocklb} shows nothing is
lost by not trying.

\emph{Summing.} Using $|C_j|\le K'$ uniformly and then H\"older with
exponents $3$ and $3/2$ on $(1,\dots,1)$ and $(L_j^{2/3})_j$,
\begin{equation}
\sum_{j=1}^{m}L_j^{2/3}
...
\label{eq:holder}
\end{equation}
by $m\le K'$ and $\sum_j L_j\le T$ from \Cref{lem:blocks}(i). Adding the two
contributions gives \Cref{eq:block-main} with $c=c_2$, and $K'\le\Kmax$ gives
the second form. As with the Cauchy--Schwarz step \Cref{eq:CS}, equality in
\Cref{eq:holder} needs all epochs equally long --- an adversary spreading its
reveals evenly --- and that is where the extra $K'^{1/3}$ is generated.
\end{proof}
...
gives the stronger $R\ge T-2N$, so such an $N$ is never worth using.)
\end{proof}
...
\begin{proof}
Divide by $T$ and require both terms to vanish.
\end{proof}

\subsection{The additive term is necessary}

The $\Theta(\Kmax B)$ term is not an artefact of the analysis. The
construction is $G_3$ again with the dyadic family \Cref{eq:dyadic} applied
recursively: each new type hides \emph{inside} the previous one, so one
coverage vector is optimal against all of them --- making the benchmark
perfect --- while each must still be located from scratch.
...
says nothing during $\cB_j$, so the budget is $b=0$. \Cref{lem:count} with
...
differing by $2B/(B-2)$ --- $4$ at $B=4$ and $2+o(1)$ thereafter; for
$B\le 2$ the lower bound is vacuous. Two special cases: $\Kmax=1$, $B=T$
recovers $\EE[\Regret(T)]\ge T-2$, so \Cref{thm:impossible} is the $B\to T$
limit; and $B=1$ makes the bound of \Cref{thm:block}
$O(n\Kmax^{4/3}T^{2/3}\log^{1/3}(n\Kmax)+\Kmax)$, the partial-information
analogue of \Cref{thm:main} and, to our knowledge, the first sublinear bound
for an unknown attacker set under partial-information feedback.
\end{remark}
...
The experiments of \Cref{sec:exp} ran at $n=3$ targets, limited not by the
...
\Cref{thm:blocklb}, exhibited separately in \Cref{fig:impossible}(d).
...
types, but because the defender cannot name the one it faces.
\Cref{thm:bits} prices this in a currency a security operations centre can
use: given any side channel emitting $b$ bits over the horizon, the optimal
regret is $\min\{T,\log_2 M-b\}\pm 3$. Regret is the number of bits still
needed to name the attacker; it is linear precisely when that number exceeds
the horizon, and on an infinite universe no finite budget reduces it at all.
...
is about \emph{latency}: even unlimited bits, delivered on a calendar of
period $B$, still cost $\Theta(\Kmax B)$, because the defender plays blind
between a type's arrival and the report naming it. \Cref{prop:adaptive} shows
these are genuinely different resources --- in $G_3$, letting the defender
choose \emph{when} the same $\Kmax$ reports arrive collapses the latency cost
from $T-2\Kmax$ to $\Kmax$. Two views from the online-learning literature
place these results: \Cref{thm:bits} is a security-game analogue of
\emph{label efficient prediction} \citep{cesabianchi2005}, except that there
the budget limits how \emph{often} the learner may look whereas
\Cref{def:budget} limits how \emph{much} it may be told, so the rate is
governed by $\log_2|\Univ|$ rather than by the number of experts; and the
additive $\Theta(\Kmax B)$ is a delay phenomenon of the kind studied under
delayed feedback \citep{joulani2013}, except that the delay applies to the
\emph{identity} of the opponent rather than to the loss.
...
\Cref{thm:bits} pins the optimum at $\min\{T,\log_2 M-b\}\pm 3$ for
$\Kmax=1$, because a miss then halves the version space unambiguously. For
$\Kmax\ge 2$ the defender does not know which opponent produced the
observation, so the search is corrupted. \Cref{prop:sqrtT} settles one half
--- the answer must contain $\Omega(\sqrt T)$, already at $M=2$ --- and the
two upper bounds we can write down, $O(\sqrt{T\Kmax(n^2+\Kmax\log n)})$ and
$O(T^{2/3}nM\log^{1/3}(nM))$, contain no $\log M$ while the lower bound
contains no $M$ at all. The joint dependence is open.

\paragraph{One theorem for volume and latency together.}
\Cref{thm:bits} charges for bits and ignores timing; \Cref{thm:blocklb}
charges for timing and lets bits be free. Neither subsumes the other, and a
defender buying threat intelligence pays for both. The missing statement is a
lower bound in a pair $(b,\lambda)$ --- budget and worst-case delay between a
type's first attack and the message naming it --- degrading to
\Cref{thm:bits} at $\lambda=0$ and to \Cref{thm:blocklb} at $b=\infty$. We
expect $\Omega(\min\{T,\log_2 M-b\}+\Kmax\lambda)$, but the two arguments
resist being run at once: \Cref{lem:count} conditions on a transcript that
the latency argument needs to keep random. A related gap is the oracle whose
messages are unbounded in length but weak in content --- one reporting the
utilities of a uniformly random element of $D_\nu$, say --- which
\Cref{thm:bits} does not cover, its budget being a bound on message length.
...
\appendix
\section{Deferred proofs and details}

\subsection{The clean form of the counting lemma}
\label{app:count-clean}

We derive \Cref{eq:count-clean} from \Cref{eq:count-final}. Write
$L=\lfloor\log_2 M\rfloor-b$ and $X=\min\{\tau-1,T\}$. If $L\le 0$ the claim
is vacuous, since $X\ge 0$.

If $0<L$ and $T\le L$, then $M\ge 2^{\lfloor\log_2 M\rfloor}=2^{L+b}$, so the
subtracted term in \Cref{eq:count-final} is at most
$2^{b}2^{T+1}/2^{L+b}=2^{\,T+1-L}\le 2$, giving $\EE[X]\ge T-2$.

If $0<L<T$, let $h=\lfloor L\rfloor$ and $f=L-h\in[0,1)$. Then $h\le T$ is an
integer and $X\ge\min\{\tau-1,h\}$ pointwise, so \Cref{eq:count-final} at
horizon $h$, with $M\ge 2^{L+b}$, gives
$\EE[X]\ge h-2^{b}2^{h+1}/M=h-2^{\,1-f}$. Now $2-f\ge 2^{\,1-f}$ on
$[0,1)$: the two sides agree at $f=0$, and the left falls with slope $-1$
while the right falls with slope $-2^{\,1-f}\ln 2\le-\ln 2>-1$. Hence
$\EE[X]\ge h+f-2=L-2$. In all cases $\EE[X]\ge\min\{T,L\}-2$. For integer $b$
and $M$ a power of two, $f=0$ and this is the plain $2^{\,T+1-L}\le 2$ of the
previous case.

\subsection{The random-walk bound of \Cref{prop:sqrtT}}
\label{app:walk}
