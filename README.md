...
* an exact bit price for the side information that repairs it: on a universe
  ...
  whole horizon, the optimal regret is `min{T, log2 M - b} +- 2`;
* the BLOCK-GROW-CATALOGUE algorithm, which recovers sublinear regret from a
  ...
  report times;
* tractable experiments at n = 10 and n = 20.
...
## Directory layout
    ...
    code/
      ...
      oracle.py                 the best-response oracle of Section 11.1:
      ...
      bench_oracle.py           Table 3 (enumeration vs oracle runtimes)
      plot_tractable_regret.py  Figure 3 (Algorithm 1 at n = 10, 20)
      plot_block_revelation.py  Figure 4 (Algorithm 2 at n = 10, 20)
      plot_impossibility.py     Figure 5 (Theorems 13, 16, 32 and
                                Propositions 21, 34 in simulation)
      verify_theory.py          re-runs every numerical claim in Sections
                                9-11 and prints PASS/FAIL
...
`project_report.tex`; the extension is appended before the bibliography.
