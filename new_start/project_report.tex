...
\begin{itemize}[leftmargin=1.1em,itemsep=0.10em,topsep=0.2em]
  ...
  \item \textbf{Algorithm (\Cref{sec:alg}).}
    ...
    expert set. Two implementation variants (total recomputation and
    incremental update) are given; they yield identical regret and differ
  ...
  \item \textbf{Main theorem and a complete, detailed proof
    ...
    The rate matches \citeauthor{balcan2015}'s
    Theorem~5.1 with $k$ replaced by $\Kmax$; for constant $\Kmax$ it
    is $\tilde O(n\sqrt T)$.
...
cardinality} question that we pose here. To the best of our knowledge, the
present work gives the first no-regret guarantee for that setting.

% ==========================================================================
\section{Model}
\label{sec:model}

We adopt the repeated-SSG notation of \citet[\S 2]{balcan2015}; our
notation is compatible with theirs so that all cross-references below are
unambiguous.
...
the set of valid coverage probability vectors; $\cP$ is a convex polytope
\citep[\S 2]{balcan2015}. Every coverage vector $\pvec$ is realizable by
...
The following definitions, taken from \citet[\S 4]{balcan2015}, are
...
The key structural lemmas we inherit are the following two of
\citet{balcan2015}, paraphrased for clarity; we cite their paper
explicitly at each use. Proposition~\ref{prop:hedge} is the standard
Polynomial-Weights guarantee adapted to the present loss range.
...
variant of Polynomial Weights / Hedge; see \citealp[\S 2]{cesa2006}) whose
...
\begin{remark}[Benchmark well-defined]
The realized sequence $\avec$ contains at most $\Kmax$ distinct types,
so $\pvec^{*}$ is always a single fixed element of $\cP$. The defender's
ignorance of $K$ affects only the algorithm, not the benchmark.
\end{remark}
...
\citet[\S 5]{balcan2015} on the catalogue $C_t$, pretending that $C_t$
...
\begin{itemize}[leftmargin=1.1em,itemsep=0.05em,topsep=0.2em]
  ...
  change. This is the standard anytime schedule; see \citet[\S 2]{cesa2006}.
...
We implement two variants of line~10: \emph{total recomputation} rebuilds
$\cE(C;\epsilon)$ from scratch, while \emph{incremental update} reuses
PW weights for extreme points that survive the new refinement. Both
variants are observationally equivalent up to numerics (they maintain the
same $\cE$ and the same distribution), and hence yield identical regret.
The incremental variant is more time-efficient when $|C|$ is not too small.
...
We prove \Cref{thm:main} in detail in \Cref{sec:proof}. Two immediate
corollaries are useful.

\begin{corollary}[Regime of applicability]
\label{cor:regime}
The bound \Cref{eq:main} is sublinear in $T$ if and only if
...
attacker types actually observed. Then \Cref{alg:gc} achieves
\[
\EE[\Regret(T)]\;\le\;O\!\Bigl(\sqrt{T\cdot K'\cdot(n^2+\Kmax\log n)}\Bigr).
\]
\end{corollary}

Both corollaries follow directly from the proof in \Cref{sec:proof}:
\Cref{cor:regime} by setting the leading term to $T$ and solving for
$\Kmax$, and \Cref{cor:adaptive} by using $K'$ rather than $\Kmax$ in the
Cauchy--Schwarz step.
...
This section is the technical heart of the report. The proof closely
tracks the structure of the known-$K$ proof of \citet[\S 5]{balcan2015}
...
\end{align}
The use of an \emph{anytime} learning rate in \Cref{alg:gc} is essential
here: the length $L_i$ of the epoch is not known at the start of
$\mathrm{Ep}_i$, since the algorithm cannot predict when the next new type
will appear. With a horizon-tuned learning rate one would either need an
a priori guess of $L_i$, or pay an extra factor in the regret bound.

\paragraph{Step 4 (Within-epoch approximation via \Cref{lem:43}).}
By \Cref{eq:epoch-types}, the sub-sequence
$(a_t)_{t\in\mathrm{Ep}_i}$ has length $L_i$ and lies in $C_i$. Apply
\Cref{lem:43} with $T'=L_i$ and catalogue $C_i$:
\begin{align}
&\max_{\pvec\in\cE_i}\!\sum_{t\in\mathrm{Ep}_i}\!U_d(b_{a_t}(\pvec),\pvec)\notag\\
&\quad\ge\;\max_{\pvec\in\cP}\!\sum_{t\in\mathrm{Ep}_i}\!U_d(b_{a_t}(\pvec),\pvec)-2\epsilon L_i.
\label{eq:lem43-epoch}
\end{align}
Since $\pvec^{*}\in\cP$, the right-hand side is at least the value at
$\pvec^{*}$ restricted to the epoch:
\begin{align}
&\max_{\pvec\in\cE_i}\!\sum_{t\in\mathrm{Ep}_i}\!U_d(b_{a_t}(\pvec),\pvec)\notag\\
...
\end{align*}
for an absolute constant $c_3$. Since the leading
$\sqrt{T\Kmax(n^2+\Kmax\log n)}$ term dominates the additive $2\Kmax+2\sqrt T$
whenever $T\ge\Kmax$ (the only regime in which the bound is non-trivial,
per \Cref{cor:regime}),
\[
...
\paragraph{Where the proof diverges from \citet{balcan2015}.}
Two places. Step~6, the discovery-round term, is new: in the known-$K$
proof there are no ``discovery rounds'' because every type is available at
$t=1$, and the additive $2\Kmax$ simply does not appear. Step~9, the
Cauchy--Schwarz step, is also new: in the known-$K$ proof the PW
sub-routine is never restarted, so there is a single $\sqrt T$ factor
instead of our $\sum_i\sqrt{L_i}$. The trade-off between these two
modifications is what produces the clean replacement of $k$ by $\Kmax$
in the final bound.
...
We implement \Cref{alg:gc} in Python (NumPy, SciPy, Matplotlib), along
with two reference algorithms: the known-$K$ oracle of
\citet[\S 5]{balcan2015}, which receives the full set $K$ at $t=1$; and a
...
\begin{figure}[t]
\centering
\includegraphics[width=0.98\linewidth]{figures/presentation/regret_vs_T_scaling.pdf}
\vspace{-0.5em}
...
case). This is entirely consistent with \Cref{thm:main}, which is an
upper bound: any instance on which the bound is tight would need to be
specifically adversarial in the sense of the lower-bound construction of
\citet[\S 7]{balcan2015}, not a randomized benign adversary.

\Cref{tab:comparison} compares the four algorithms at $T=4000$. The regret
of \textsc{Grow-Catalogue} (both total and incremental variants) is
...
Three directions for future work are immediate. The most important is the
partial-information (bandit) extension; this requires an adaptive version
of the barycentric-spanner construction of \citet[\S 6]{balcan2015},
since the spanner dimension changes whenever a new type is discovered.
A second direction is the adaptive-adversary case under partial feedback,
which is an open problem even in the known-$K$ setting. Finally, the
Cauchy--Schwarz step (Step~9 of the proof) loses a $\sqrt{\Kmax}$ factor
in the worst case; sleeping-experts techniques \citep{mourtada2017} may
remove this when $K'\ll\Kmax$, tightening the instance-adaptive bound of
\Cref{cor:adaptive}.

\paragraph{Reproducibility.}
Our Python implementation and all experiment scripts are at
\textcolor{blue}{\href{https://github.com/CS6002-SSG/grow-catalogue}{github.com/CS6002-SSG/grow-catalogue}}. The experiments
in \Cref{sec:exp} can be reproduced by running
\texttt{python plot\_presentation.py} and
\texttt{python plot\_regret\_over\_sqrtT.py} after installing the
dependencies in \texttt{requirements.txt}.
...
appendix records low-level implementation details useful for
reproducibility.

\paragraph{Computing $\cE(C;\epsilon)$.}
For $n\le 4$ we enumerate all $(n-1)$-subsets of the hyperplanes defining
the partition of $\cP$, solve the resulting linear systems, and keep
each intersection point that lies in $\cP$. The set of hyperplanes
consists of (i) $n$ simplex constraints ($p_i\ge 0$ for each $i$ and the
outer bounding hyperplane of $\cP$), and (ii) $|C|\binom{n}{2}$
pairwise best-response constraints, one per pair of targets per attacker
type. For $n=3,\Kmax=4$ this gives at most $4+4\cdot 3=16$ hyperplanes
and $\binom{16}{2}=120$ candidate intersections, which is tractable.

\paragraph{Anytime learning rate.}
The schedule $\eta_t=\sqrt{\log|\cE|/t}$ \citep[\S 2]{cesa2006} is
necessary for the $\sqrt{L_i\log|\cE_i|}$ per-epoch bound of Step~3 to
hold, since $L_i$ is not known at the start of $\mathrm{Ep}_i$.

\paragraph{Hyperparameters.}
Unless stated otherwise, the experiments use $n=3$, $R=1$, $\Kmax=3$ or
$4$, $T\in\{4000,6000\}$, and averaging over 40--80 seeds. The rolling
window in \Cref{fig:per-round} is $75$ rounds. Dependencies:
\texttt{numpy>=1.26}, \texttt{scipy>=1.11}, \texttt{matplotlib>=3.8}.
Both the code and a \texttt{requirements.txt} are at the GitHub repository
linked in \Cref{sec:conc}.
